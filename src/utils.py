...
def render_key_frame(
    parent: tk.Widget, allowed_letters: List[str], Color, key_font=("Helvetica", 10)
) -> Tuple[tk.Frame, Dict[str, tk.Label]]:
...
def render_guess_entry_tiles(
    parent: tk.Widget,
    allowed_word_length: int,
    allowed_number_of_guesses: int,
    Color,
    entry_font=("Helvetica", 20),
) -> Tuple[tk.Frame, List[List[tk.Entry]]]:
...
def validate_guess(
    ...
    for char in guess:
        if char not in allowed_letters:
            messagebox.showinfo(
                "Ugyldig bokstav", f"Bokstaven '{char}' er ikke tillatt."
            )
            return False
